...
const ForgotPasswordPage = ({errorMessage,forgotPasswordFunction,title,description,backFunction,quote,author,credential}:ForgotPasswordPageProps) => {
  return (
    <div className='min-h-screen grid grid-cols-1 lg:grid-cols-2 '>
        <div className='flex items-center justify-center bg-gradient-to-br from-primary to-black dark:bg-gradient-to-br'>
            <Suspense fallback={<LoadingCard title={title} description={description}/>}>
                <ForgotPasswordCard forgotPasswordFunction={forgotPasswordFunction} backFunction={backFunction} errorMessage={errorMessage}/>
            </Suspense>
        </div>
